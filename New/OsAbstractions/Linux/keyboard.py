...
from common import EV_KEY, aggregate_devices

from New.Temp._keyboard_event import KeyboardEvent, KEY_DOWN, KEY_UP
from New.Temp._canonical_names import all_modifiers, normalize_name
...
def build_tables():
    ...
    try:
        ...
        raise ValueError(
            'Failed to run dumpkeys to get key names.' 
            'Check if your user is part of the "tty" group,' 
            'and if not, add it with "sudo usermod -a -G tty USER".'
            ) from e
    ...
    for str_scan_code, str_names in re.findall(keycode_template, dump, re.MULTILINE):
        ...
        for i, str_name in enumerate(str_names.strip().split()):
            modifiers = tuple(sorted(
                modifier for modifier, bit in modifiers_bits.items() if i & bit
                ))
...
def listen(callback):
    ...
    while True:
        time, event_type, code, value, device_id = device.read_event()
