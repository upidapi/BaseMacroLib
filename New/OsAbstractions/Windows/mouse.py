"""
abstractions for comunicating with the windows mouse
"""
...
class MSLLHOOKSTRUCT(Structure):
    """ python repsentation of the windows MSLLHOOKSTRUCT """
    _fields_ = [("x", c_long),
                ("y", c_long),
                ('data', c_int32),
                ('reserved', c_int32),
...
def listen(queue):
    def low_level_mouse_handler(nCode, wParam, lParam):
        ...
        if wParam == WM_MOUSEMOVE:
            ...
            event_type, button = buttons_by_wm_code.get(wParam, ('?', '?'))
            if wParam >= WM_XBUTTONDOWN:
                button = {0x10000: X, 0x20000: X2}[struct.data]
            event = ButtonEvent(event_type, button, t)
...
class POINT(Structure):
    """ python repsentation of the windows POINT """
