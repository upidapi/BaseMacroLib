...
from New.Temp._mouse_event import (
    # ButtonEvent,
    # WheelEvent,
    # MoveEvent,
    LEFT,
...
def press(button=LEFT):
    ...
    if \
        _last_click["time"] is not None and \
        datetime.datetime.now() - _last_click["time"] < datetime.timedelta(seconds=0.3) and \
        _last_click["button"] == button and \
...
def release(button=LEFT):
    ...
    # now - 0.3s
    x = datetime.datetime.now() - datetime.timedelta(microseconds=300000)

    if \
        _last_click["time"] is not None and \
        _last_click["time"] > x and \
        _last_click["button"] == button and \
        _last_click["position"] == location:
...
def move_to(x, y):
    ...
    for b, button_state in _button_state.items():
        if button_state:
            e = Quartz.CGEventCreateMouseEvent(
                None,
